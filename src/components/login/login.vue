...
<template>
  <div id="login" class="comBox">
    <g-mainBox bottom="1rem" @keyup.native.enter="login" style="height: 100%;">
      ...
      <g-input v-model.trim="loginForm.username" class="gInputClass" clearable placeholder="账号名称">
        <i class="yiyifont yiyi-portrait" slot="prefix"></i>
      </g-input>
      <g-input v-model.trim="loginForm.password" class="gInputClass" :type.sync="inputType" pwd clearable placeholder="密码">
        <i class="yiyifont yiyi-password" slot="prefix"></i>
      </g-input>

      <div style="display: flex;align-items: center;">
        <g-input v-model.trim="picLyanzhengma" class="gInputClass imgCode" clearable placeholder="验证码">
          <i class="yiyifont yiyi-password" slot="prefix"></i>
        </g-input>
      ...
      <div class="checkIcon">
        <check-icon :value.sync="selected" type="plain">
          <label class="agree" style="color: white;">我已阅读并同意</label>
        </check-icon>
        <a class="agree" style="text-decoration: none;line-height: .86rem;" href="javascript:;" @click="show_agm">《合作协议》</a>

      </div>
      <x-button type="primary" :show-loading="btnLoding" :disabled="btnDisabled || loginBtnDisabeld" class="loginBtn" @click.native="login">登 录
      </x-button>
